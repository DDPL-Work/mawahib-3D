...
function Footer() {
  ...
  const sections = [
    ["Product", ["Pricing", "Request Demo"]],
    ["Resources", ["How it Works", "Dashboard"]],
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto" }}>
        <div style={{
          ...
          <div style={{
            ...
            gap: isMobile ? 24 : 48, justifyContent: isMobile ? "center" : "flex-end"
          }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {items.map((item) => {
                    const paths = {
                      ...
                      Privacy: "/privacy",
                      Dashboard: "/dashboard",
                      "How it Works": "/how-mawahib-works",
                      "Request Demo": WHATSAPP_URL,
                      Careers: "#",
                      Terms: "#",
                      "Contact Us": WHATSAPP_URL,
                      Features: "/#features",
                      "Help Center": "#",
                      "Cookie Policy": "#"
                    };
                    const isExternal = paths[item] && (paths[item].startsWith("http") || paths[item] === "#" || paths[item].startsWith("/#"));

                    return paths[item] ? (
                      <motion.div key={item} whileHover={{ color: "rgba(255,255,255,.9)", x: 2 }} transition={{ duration: 0.2 }}>
                        {isExternal ? (
                          <a href={paths[item]} target={paths[item] === "#" ? "_self" : "_blank"} rel="noopener noreferrer" style={{ color: "rgba(255,255,255,.65)", fontSize: 13, textDecoration: "none" }}>{item}</a>
                        ) : (
                          <Link to={paths[item]} style={{ color: "rgba(255,255,255,.65)", fontSize: 13, textDecoration: "none" }}>{item}</Link>
                        )}
                      </motion.div>
                    ) : null;
                  })}
        ...
        <div style={{
          ...
          <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 10 }}>
            {["AI-Powered", "SOC 2 Type II", "GDPR Ready"].map((b) => (
              <span key={b} style={{ fontSize: 9.5, fontWeight: 600, color: "rgba(255,255,255,.5)", border: "1px solid rgba(255,255,255,.08)", borderRadius: 9999, padding: "4px 12px", letterSpacing: ".06em", background: "rgba(255,255,255,0.02)" }}>{b}</span>
            ))}
          </div>
        </div>
      </div>
